...
#
# PICO 闭环一键启动 + 自动核验（对比验证基线脚本），支持 IsaacLab / MuJoCo 两后端
#
...
#   （isaaclab 后端还要在 IsaacLab 窗口按 U 解锁）
...
case "$BACKEND" in
  ...
  *) echo "❌ 未知 backend: $BACKEND（支持 isaaclab|mujoco）" >&2; exit 2 ;;
esac
...
snapshot() {
  ...
  [[ "$BACKEND" == isaaclab ]] && repos+=("$ISAACLAB_ROOT_DIR")
  local d
  for d in "${repos[@]}"; do
  ...
  [[ "$BACKEND" == isaaclab ]] && bins+=("$PROXY_BIN")
  local b
  for b in "${bins[@]}"; do
    if [[ -x "$b" ]]; then
...
run_checks() {
  if [[ "$BACKEND" == isaaclab ]]; then run_checks_isaaclab; else run_checks_mujoco; fi
}
