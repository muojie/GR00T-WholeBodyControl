...
`--source bvh` 是本地文件回放输入源。manager 直接读取 `--bvh-file`，换文件需要重启 manager。完整说明见 [Sony mocopi / BVH-G1 POSE v1 路线](mocopi_pose_bvh_g1_v1_route.md) 的 BVH 输入源章节。
...
`--source bvh_stream` 是在线 UDP skeleton stream 输入源。manager 只监听 UDP，不绑定具体 BVH 文件；换动作只重启 sender。完整说明见 [Sony mocopi / BVH-G1 POSE v1 路线](mocopi_pose_bvh_g1_v1_route.md) 的 BVH 输入源章节。
