# Sony mocopi / `--source bvh` 本地 BVH 文件回放

本文只记录 `mocap_manager_server.py --source bvh`。它和 [`--source bvh_stream`](mocopi_source_bvh_stream.md) 是两个不同输入源，不应互相覆盖。
...
这不是当前 MuJoCo 主验证命令。当前主验证路线使用 [`--source bvh_stream`](mocopi_source_bvh_stream.md) 或 `--source bvh_g1` 生成 G1 joint reference。
