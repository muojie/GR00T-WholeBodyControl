...
show_usage() {
    ...
    echo "  --udp-out-host HOSTS    Set UDP output destination host(s), comma-separated, with optional :port (default: $UDP_OUT_HOST_DEFAULT)"
...
case "$G1_LOCAL_ROBOT_ID_DEFAULT" in
    2)
        ...
        UDP_OUT_HOST_DEFAULT="${G1_2_UDP_OUT_HOSTS:-${G1_2_UDP_OUT_HOST:-${G1_UDP_OUT_HOSTS:-${G1_UDP_OUT_HOST:-$WINDOWS_ISAACLAB_IP_DEFAULT}}}}"
        ...
        # host 双机器人：第二套 deploy 的 DDS 话题前缀（rt/r2/lowcmd 等），
        # 与 Isaac 侧 g129_r2 通道对接。外部已 export 时以外部为准。
        SONIC_DDS_TOPIC_PREFIX_DEFAULT="${G1_2_DDS_TOPIC_PREFIX:-rt/r2}"
        ;;
    *)
        ...
        UDP_OUT_HOST_DEFAULT="${G1_1_UDP_OUT_HOSTS:-${G1_1_UDP_OUT_HOST:-${G1_UDP_OUT_HOSTS:-${G1_UDP_OUT_HOST:-$WINDOWS_ISAACLAB_IP_DEFAULT}}}}"
        ...
        SONIC_DDS_TOPIC_PREFIX_DEFAULT="${G1_1_DDS_TOPIC_PREFIX:-rt}"
        ;;
esac
SONIC_DDS_TOPIC_PREFIX="${SONIC_DDS_TOPIC_PREFIX:-$SONIC_DDS_TOPIC_PREFIX_DEFAULT}"
export SONIC_DDS_TOPIC_PREFIX
