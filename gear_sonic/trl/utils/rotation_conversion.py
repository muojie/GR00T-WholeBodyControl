# This file contains code derived from PyTorch3D (via https://github.com/Mathux/ACTOR),
# originally Copyright (c) Facebook, Inc. and its affiliates, licensed under BSD.
# See https://github.com/facebookresearch/pytorch3d/blob/main/LICENSE for the original license.

"""PyTorch 3D rotation conversions (quaternion, rotation matrix, axis-angle, 6D).

Provides batched, differentiable conversions between quaternion (wxyz),
rotation matrix, axis-angle, and 6D rotation representations, plus
swing-twist decomposition and yaw extraction.
...
def _sqrt_positive_part(x):
    ...
    """
    return torch.where(x > 0, torch.sqrt(torch.clamp_min(x, 1e-5)), 0)
...
def axis_angle_to_quaternion(axis_angle):
    ...
    sin_half_angles_over_angles = torch.where(
        angles.abs() < eps, 0.5 - (angles * angles) / 48, torch.sin(half_angles) / angles
    )
