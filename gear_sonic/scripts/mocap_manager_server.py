    ...
    *,
    ...
    if args.source in {"pkl", "bvh_g1", "bvh_stream"}:
        if args.control_mode != "pose":
            ...
            planner_from_root_ok = args.planner_from_root
            if not planner_follow_ok and not planner_from_root_ok:
                parser.error(
                    f"--source {args.source} requires --control-mode pose "
                    "(or planner with --planner-follow-stream-root or --planner-from-root for bvh_stream)"
                )
